-- Ticker Alpha — asset-kind filter on search
--
-- Apply after 0061. Safe to re-run.
--
-- The nav search gained an asset-type dropdown (All / Stocks / ETFs / Funds /
-- Crypto). Filtering the RPC's dozen rows in the browser starved narrow
-- kinds: "sp" under ETFs returned nothing because all twenty-five rows the
-- ranking produced were stocks, with SPY ranked below them. The filter has to
-- run before the limit, so it lives here. p_kind null or unrecognised means
-- no filter, which is what the dropdown's "All" sends and what an older page
-- falls back to. "fund" folds in money markets, matching the page's reading
-- of the word.
--
-- Identical to 0052's function otherwise; generated from it, not retyped.
...
do $$
begin
  -- The two-argument overload must go, or calls that omit p_kind are ambiguous.
  begin
